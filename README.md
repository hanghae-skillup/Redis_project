...
## 1. Lease Time
`leaseTime`은 락이 유지되는 최대 시간으로 특정 조건에서 락 해제를 보장하기 위해 설정.

- **설정 값**: `leaseTime = 10초`
- **설정 이유**:
  1. **비정상 종료 방지**
    - 작업 중 애플리케이션이 비정상적으로 종료되거나 네트워크 장애가 발생하더라도 락이 해제되지 않아 무한 대기가 발생할 수 있음.
    - `leaseTime`은 이러한 상황을 방지하기 위해 설정. 지정된 시간이 지나면 락이 자동으로 해제됨.
  2. **작업 소요 시간 기준**
    - 좌석 예약 로직은 일반적으로 1~3초 내에 처리된다고 가정함.
    - 안정성을 위해 충분히 여유를 두어 10초로 설정함.

---

## 2. Wait Time
`waitTime`은 락을 획득하기 위해 대기할 최대 시간을 의미합니다.

- **설정 값**: `waitTime = 0초`
- **설정 이유**:
  1. **즉시 실패 처리**
    - 사용자는 좌석 예약 시 응답이 실시간으로 올것을 기대함.
    - 락을 획득하지 못할 경우 즉시 실패하도록 설정하여 사용자 경험(UX)을 향상시킴.
  2. **동시 요청 처리가 많은 환경을 고려**
    - 여러 사용자가 동시에 같은 좌석을 예약하려고 시도하는 경쟁 상황에서 무한 대기를 방지함.
    - 일정 시간 대기 후 실패로 처리하여 다른 프로세스가 락을 획득할 기회를 제공함.